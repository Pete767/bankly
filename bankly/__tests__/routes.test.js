...
describe("POST /auth/register", function() {
  ...
  test("verify with different secret key should fail", async function() {
    const response = await request(app)
      .post("/auth/register")
      .send({
        username: "new_user",
    ...
    //Tests Bug #4
    expect(() => {
      jwt.verify(response.body.token, '5555');
   }).toThrow(Error);
  });
...
describe("POST /auth/login", function() {
  ...
  // TESTS BUG #3
  test("check for admin flag for admin account", async function() {
    const response = await request(app)
      .post("/auth/login")
      .send({
        username: "u3",
...
describe("GET /users", function() {
  ...
  // TESTS BUG #1
  test("should list all users with basic info", async function() {
    const response = await request(app)
      .get("/users")
      .send({ _token: tokens.u1 });
    expect(response.statusCode).toBe(200);
...
describe("GET /users/[username]", function() {
  ...
  // TESTS BUG #2
  test("should return no such user", async function() {
    const response = await request(app)
      .get("/users/nobody")
      .send({ _token: tokens.u1 });
    expect(response.statusCode).toBe(200);
